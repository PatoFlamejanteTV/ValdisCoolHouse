...
local PlayerBlockedEvent = StarterGui:GetCore("PlayerBlockedEvent")
local PlayerUnblockedEvent = StarterGui:GetCore("PlayerUnblockedEvent")
local PlayerMutedEvent = StarterGui:GetCore("PlayerMutedEvent")
local PlayerUnmutedEvent = StarterGui:GetCore("PlayerUnmutedEvent")
local PlayerFriendedEvent = StarterGui:GetCore("PlayerFriendedEvent")
local PlayerUnfriendedEvent = StarterGui:GetCore("PlayerUnfriendedEvent")
...
local DEVELOPER_ICON = 'rbxasset://textures/ui/PlayerList/Developer.png'
...
local function getFriendStatusIcon(friendStatus) -- i removed completly frined status along with friend rquest icon
    ...
    return ""
	  ...
	  pcall(function()
		...
		for _, blockedUserId in ipairs(blockedUserIds) do
		  local player = PlayersService:GetPlayerByUserId(blockedUserId)
		  if player ~= nil then
			return BLOCKED_ICON
		  end
		end
...
if isTenFootInterface then
  ...
  Container.Position = UDim2.new(1, -167, 0, 4)
end
...
if not isTenFootInterface then
  ...
  PlayerFriendedEvent.Event:Connect(function(otherPlayer)
    onFriendshipChanged(otherPlayer, Enum.FriendStatus.Friend)
  end)
  PlayerUnfriendedEvent.Event:Connect(function(otherPlayer)
...
local function setupEntry(player, newEntry, isTopStat)
  ...
  updateLeaderstatFrames()
  updateFollowRelationship(player, newEntry, "Not")
...
local IsSmallScreenDevice = false -- TODO: implement this later or something
...
Playerlist.TopbarEnabledChanged = function(enabled)
  ...
  -- TODO onCoreGuiChanged(Enum.CoreGuiType.PlayerList, StarterGui:GetCoreGuiEnabled(Enum.CoreGuiType.PlayerList))
end
...
PlayerBlockedEvent.Event:Connect(function(otherPlayer)
  blockStatusChanged(otherPlayer.UserId, true)
end)

PlayerUnblockedEvent.Event:Connect(function(otherPlayer)
  blockStatusChanged(otherPlayer.UserId, false)
end)
...
Playerlist.ToggleVisibility() Playerlist.ToggleVisibility() -- just to fix some things
Playerlist:IsOpen()
return Playerlist